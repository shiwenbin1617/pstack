---
name: recall
description: "Recover recent work and open decisions from relevant task history when the user asks to recall or resume work."
...
Resolve `scripts/find-transcripts.mjs` relative to this `SKILL.md`. Run it with `node`, `--host claude`, and `--workspace` set to the active workspace to list this workspace's transcripts. The helper reads JSONL `cwd` metadata, matches the workspace path exactly, and sorts results by modification time. It does not inspect another workspace's message content.
...
3. Fan out across your chat history. Spawn parallel subagents on a fast, cheap model, each taking a slice of the ordered paths returned by the helper. Tell every subagent to grep the topic first, then read only matching chats and their relevant regions. Skip the current chat plus obvious noise such as subagent, eval, and test chats. Each returns the same schema, one block per chat: topic, the user's goal, decisions, open threads, struggles and corrections, and artifacts such as PRs, tickets, and branches. Cite each chat UUID. For one or two chats, skip the fan-out and search directly. The raw transcripts stay in the subagents. The main thread gets only their findings.
4. Follow shared records when a linked issue, incident, or unresolved historical question affects the requested brief. Use the **why** skill's relevant source playbook. Do not sweep every source merely because the task names a feature or file. Stop when the requested context is recovered or no useful accessible lead remains.
