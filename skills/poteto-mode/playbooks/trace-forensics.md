...
1. Identify the format and load it with an appropriate parser. Delegate independent analysis only when useful.
2. Query the artifact with existing tools. Convert it to another format only when that makes the required analysis easier.
...
6. Hand back a cited diagnosis. Continue to Bug fix or Perf issue only when implementation is already authorized.
