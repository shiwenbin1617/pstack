...
1. Locate the prior trail. For a local transcript, resolve `../recall/scripts/find-transcripts.mjs` relative to this `SKILL.md`. Run it with `node`, `--host claude`, and `--workspace` set to the active workspace. The helper reads JSONL `cwd` metadata and returns only exact workspace matches in modification-time order. The trail can instead be a subagent's returned report or a pushed branch. Read the metadata overview and last messages first, then scan back for the decision points. Parse a long transcript in a subagent and keep the reduced timeline in the main thread (the **principle-guard-the-context-window** skill).
