...
6. **Verify the chain from transcripts, not self-report.** Resolve `../recall/scripts/find-transcripts.mjs` relative to this `SKILL.md`. Run it with `node`, `--host claude`, and `--workspace` set to the active workspace. Read each candidate's local transcript from the returned paths. The helper scopes results through exact JSONL `cwd` metadata. Look at which files each candidate opened. Citing a principle is not reading its leaf skill, and reading it is not applying it. Grade chain-following from the files it read and the shape of the code, never from the candidate's claims.
