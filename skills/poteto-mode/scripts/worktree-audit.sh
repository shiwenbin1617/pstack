...
# Search only host-native transcript stores. The worktree path match below keeps
# Codex's global date-partitioned store scoped to this repository.
slug=$(printf '%s' "$main_wt" | sed 's#/#-#g')
transcript_roots=()
[ -d "$HOME/.claude/projects/$slug" ] && transcript_roots+=("$HOME/.claude/projects/$slug")
[ -d "$HOME/.codex/sessions" ] && transcript_roots+=("$HOME/.codex/sessions")
now=$(date +%s)
