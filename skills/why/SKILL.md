...
Before spawning investigators, list the MCP tools actually present in this session. They appear in your tool list namespaced `mcp__<server>__<tool>`; that list is the dependable source. `claude mcp list` names the configured servers when you need the fuller picture.
...
- `model`: your configured why-investigators model (defaults to your fast code model)
- read-only, but with MCP access intact, so MCP-backed investigators still work and "don't change anything" is enforced by the harness rather than by the prompt. On Claude Code that is `subagent_type: Explore`.
...
- a general-purpose subagent
- `model`: your configured why-synthesizer model (defaults to your judgment model)
- read-only, with MCP access intact. The synthesizer's quality check spot-verifies citations, which can require MCP. On Claude Code that is `subagent_type: Explore`.
